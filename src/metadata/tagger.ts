...
// Define the shape of a user defined text entry to match NodeID3's expectations
...
async function applyMp3Metadata(filePath: string, metadata: AudiobookMetadata): Promise<boolean> {
    try {
        ...
        // Create an array for user defined text entries
        let userDefinedEntries: UserDefinedText[] = [];

        // Add series information using custom TXXX frames
        if (metadata.series) {
            userDefinedEntries.push({
                description: 'Series',
        ...
        // Only assign userDefinedText if we have entries
        if (userDefinedEntries.length > 0) {
            // NodeID3 expects an array for userDefinedText, but TypeScript is being strict about its shape
            // Use type assertion to satisfy TypeScript while maintaining compatibility with NodeID3
            tags.userDefinedText = userDefinedEntries as any;
        }
